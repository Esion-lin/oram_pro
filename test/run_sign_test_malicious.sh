...
for scenario in "${SCENARIOS[@]}"; do
    ...
    do
        ...
        # 运行三个子任务并将输出附加到文件
        ./test_sign_malicious player0 $data_size >> "${NAME}_malicious_output_player0.log" 2>&1 &
        ./test_sign_malicious player1 $data_size >> "${NAME}_malicious_output_player1.log" 2>&1 &
        ./test_sign_malicious player2 $data_size >> "${NAME}_malicious_output_player2.log" 2>&1 &
        wait
