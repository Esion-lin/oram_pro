...
void initializeCommunication( int p);
...
template<typename T>
void sendVector(T* vec, size_t player, size_t size)
{
	if(!communicationSenders[player]->sendMsg(vec, size * sizeof(T), 0))
		std::cout << "Send vector error" << endl;
}

template<typename T>
void receiveVector(T* vec, size_t player, size_t size)
{
	...
	if(!communicationReceivers[player]->receiveMsg(vec, size * sizeof(T), 0))
		std::cout << "Receive myType vector error" << endl;
}
