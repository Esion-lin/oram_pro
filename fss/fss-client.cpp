...
void generateTreeEq_32(Fss* f, ServerKeyEq* k0, ServerKeyEq* k1, uint64_t a_i, uint32_t b_i){
    ...
    uint64_t sInt0 = 0, sInt1 = 0;

    for(int i = 0; i < 4; i ++){
        // printf("k0:%llu, k1:%llu\t", key0[i],key1[i]);
        sInt0 += key0[i];
        sInt1 += key1[i];
        if(i != 3){
            sInt0<<=8;sInt1<<=8;
    ...
    if (sInt0 != sInt1) {
        uint64_t diff = b_i - sInt0 + sInt1;
        if(tbit0 == 0) diff = -diff;

        mpz_import(k0->w.get_mpz_t(), 1, 0, sizeof (uint64_t), 0, 0, &diff);
        k1->w = k0->w;
    } else {
        printf("-----------------------ERROR!-------------------------\n");
        k0->w = 0;
