...
int main(){
    ...
    EC_POINT *H, *res = EC_POINT_new(curve), *res_2 = EC_POINT_new(curve), *temp = EC_POINT_new(curve);;
    BIGNUM *x = BN_new(), *r = BN_new();
    BIGNUM *x_2 = BN_new(), *r_2 = BN_new();
    uint8_t xx[4] = {1,2,3,4}, rr[32] = {1,2,3,4}, xx_1[4], rr_1[32];
    ...
    gettimeofday(&t1,NULL);
    for(int i = 0; i < 1000; i++)
    EC_POINT_add(curve, res_2, res, res, NULL);
    gettimeofday(&t2,NULL);
    std::cout<<(t2.tv_sec - t1.tv_sec) + (double)(t2.tv_usec - t1.tv_usec)/1000000.0<<std::endl;
